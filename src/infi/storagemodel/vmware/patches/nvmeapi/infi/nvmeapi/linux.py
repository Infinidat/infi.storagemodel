...
# TO DO CHECK RHEL HOW IT IS NAMED
if 'ubuntu' in get_platform_string() or 'suse' in get_platform_string():
    NVME_CONNECTION_CONFIG = '/etc/iscsi/nodes'
else:
    NVME_CONNECTION_CONFIG = '/var/lib/iscsi/nodes'
ISCSI_INITIATOR_IQN_FILE = '/etc/iscsi/initiatorname.iscsi'
GENERATE_COMMAND = 'iscsi-iname'

class LinuxNVNEapi(base.ConnectionManager):
    ...
    def _parse_discovery_endpoint(self, iqn):
        ...
        for end_point in os.listdir(os.path.join(NVME_CONNECTION_CONFIG, iqn)):
            filepath = os.path.join(NVME_CONNECTION_CONFIG, iqn, end_point)
    ...
    def get_discovered_targets(self):
        targets = []
        if not os.path.isdir(NVME_CONNECTION_CONFIG):
            return targets
        for iqn in os.listdir(NVME_CONNECTION_CONFIG):
            endpoints = []
            for end_point in os.listdir(os.path.join(NVME_CONNECTION_CONFIG, iqn)):
